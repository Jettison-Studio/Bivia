...
select is((public.bivia_leaderboard_v1('all')->0->>'gamesPlayed')::int,1,'Zero-score game included in completion count');
...
select is((select extract(epoch from (v->>'deadlineAt')::timestamptz-(v->>'questionStartedAt')::timestamptz)::numeric from edge_state where k='right'),25::numeric,'Timed second question gets 25 seconds');
